		...
		if(a.type=="GUANJIANZI") {//关键字
			switch(a.bh) {
				...
				case 6: {//define
					...
					y=Q.front();
					while(y.type!="GUANJIANZI") {
						Q.pop();node[t].content+=y.name;
						y=Q.front();
					}
...
//		return false ;
	...
	ll dfs(ll x,ll fro,ll c_n,ll b_n,ll g_n,ll swi) {//上一个节点是fro,进入一个x号body,返回body的最后一个节点 
		...
		for(re ll i=0;i<sx.children_num;i++) {
			...
			if(n_t.type=="if") {
				...
				cout<<"\"];"<<endl;
				if(chc1!=-1) {
					cout<<fx<<" -> "<<chc1<<" [label=\"调用函数\"];"<<endl;
					cout<<out_h[chc1]<<" -> "<<fx<<" [label=\"函数返回\"];"<<endl;
				}
				if(chc2!=-1) {
					cout<<fx<<" -> "<<chc2<<" [label=\"调用define\"];"<<endl;
					cout<<chc2<<" -> "<<fx<<" [label=\"define返回\"];"<<endl;
				}
				for(re ll j=0;j<n_t.children_num;j++) {
					ll jt=n_t.children[j];Node n_jt=ast.node[jt];
			...
			/*a [label = "this is node a"];*/
				...
				cout<<"\"];"<<endl;
				if(chc1!=-1) {
					cout<<ct<<" -> "<<chc1<<" [label=\"调用函数\"];"<<endl;
					cout<<out_h[chc1]<<" -> "<<ct<<" [label=\"函数返回\"];"<<endl;
				}
				if(chc2!=-1) {
					cout<<ct<<" -> "<<chc2<<" [label=\"调用define\"];"<<endl;
					cout<<chc2<<" -> "<<ct<<" [label=\"define返回\"];"<<endl;
				}
				...
				cout<<"\"];"<<endl;
				if(chc1!=-1) {
					cout<<t<<" -> "<<chc1<<" [label=\"调用函数\"];"<<endl;
					cout<<out_h[chc1]<<" -> "<<t<<" [label=\"函数返回\"];"<<endl;
				}
				if(chc2!=-1) {
					cout<<t<<" -> "<<chc2<<" [label=\"调用define\"];"<<endl;
					cout<<chc2<<" -> "<<t<<" [label=\"define返回\"];"<<endl;
				}
				c_n=t;b_n=end_do;
	...
	ll re_node_cnt,e_cnt;
	map<ll,string>cont;
	struct edge {
		ll fro,to,nxt;
		string content;
		edge(ll f=0,ll t=0,ll nx=0,string x="") {
			fro=f,to=t,nxt=nx,content=x;
		}
	}e[10005];
	ll head[10005];bool vis[10005];
	void adde(ll f,ll t,string w) {
		e[++e_cnt]=edge(f,t,head[f],w);
		head[f]=e_cnt;
	}
	void dfs2(ll x) {
		vis[x]=1;
		...
		for(re ll i=head[x];i;i=e[i].nxt) {
			ll t=e[i].to;
			cout<<x<<" -> "<<t<<e[i].content<<endl;
			if(!vis[t]) dfs2(t);
		}
	}
	void rebuild() {
		freopen("test.gv","r",stdin);
		memset(head,0,sizeof(head));
		string arr;ll a; 
		char ch=getchar();
		while(ch!='0') ch=getchar();a=0;
		do {
			arr.clear();cin>>arr;
			if(arr=="->") {
				ll b=read();
	...
	void build_testchart() {
		/*a [label = "this is node a"];*/
			...
			cout<<rt<<" [shape=hexagon,label=\"开始\"];"<<endl;
			for(re ll i=0;i<ast.node[0].children_num;i++) {
				...
				if(sx.type=="include") continue ;
					...
					if(ast.node[sx.children[0]].content!="main") {create_hanshu(sx);}
						...
						ll z=Node_cnt++; 
						cout<<z<<" [shape=hexagon,label=\"end\"];"<<endl;
						cout<<temp_d<<" -> "<<z<<" "<<endl;
					}
				}
				else {
